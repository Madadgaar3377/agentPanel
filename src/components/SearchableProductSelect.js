    ...
    <div ref={wrapRef} className={`relative ${className}`}>
      <div className="relative">
        <input
          ...
          className={
            inputClassName ||
            'w-full px-5 py-4 bg-white border-2 border-blue-200 focus:border-blue-500 rounded-2xl text-sm font-semibold outline-none transition-all shadow-sm pr-20'
          }
        />
        {value ? (
          <button
            type="button"
            onClick={() => pick('')}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-black uppercase tracking-widest text-blue-600 hover:text-red-600"
          >
            Clear
          </button>
        ) : (
      ...
      {open && (
        <ul className="absolute z-50 mt-2 w-full max-h-72 overflow-y-auto bg-white border-2 border-blue-200 rounded-2xl shadow-xl py-1 text-sm">
          <li>
            <button
              type="button"
              onClick={() => pick('')}
              className="w-full text-left px-4 py-3 hover:bg-blue-50 font-semibold text-blue-800 border-b border-blue-100"
            >
              {createNewLabel}
            </button>
          </li>
                ...
                <li key={id}>
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-blue-50 ${
                      active ? 'bg-blue-100 font-bold text-blue-900' : 'text-gray-800'
                    }`}
                  >
                    {getProductLabel(p)}
                  </button>
