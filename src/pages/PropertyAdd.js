...
const PropertyAdd = () => {
    ...
    const showToast = (message, type) => {
        ...
        reactToast[type === 'success' ? 'success' : 'error'](message);
    };
    ...
    // Auto-fill contact from current agent user
    useEffect(() => {
        if (user && !isEditMode) {
            const contactData = {
                name: user.name || '',
            ...
            if (propertyType === 'Project') {
                ...
                setIndividualData(prev => ({ ...prev, contact: {
                    name: user.name || '',
                    email: user.email || '',
                    number: user.phoneNumber || '',
                    whatsapp: user.WhatsappNumber || '',
            ...
            if (response.success) {
                ...
                if (property.type === 'Project' && property.project) {
                    ...
                    setProjectData({
                        ...
                        developmentType: proj.developmentType || '',
                        infrastructureStatus: proj.infrastructureStatus || '',
                        projectStage: proj.projectStage || '',
                        expectedCompletionDate: proj.expectedCompletionDate || '',
                        possessionDate: proj.possessionDate || '',
                        utilities: proj.utilities || {
                            electricity: false,
                            water: false,
                            gas: false,
                            internet: false,
                        ...
                        amenities: proj.amenities || projectData.amenities,
                        description: proj.description || '',
                        highlights: proj.highlights || ['', '', ''],
                        totalLandArea: proj.totalLandArea || '',
                        landAreaUnit: proj.landAreaUnit || 'sqft',
                        propertyTypesAvailable: proj.propertyTypesAvailable || [],
                        totalUnits: proj.totalUnits || '',
                        typicalUnitSizes: proj.typicalUnitSizes || '',
                        nearbyLandmarks: proj.nearbyLandmarks || '',
                        remarks: proj.remarks || '',
                        developerBuilder: proj.developerBuilder || '',
                        units: proj.units || [],
                        transaction: {
                            type: proj.transaction?.type || 'Sale',
                            price: proj.transaction?.price || '',
                            priceRange: proj.transaction?.priceRange || '',
                            advanceAmount: proj.transaction?.advanceAmount || '',
                            monthlyRent: proj.transaction?.monthlyRent || '',
                            contractDuration: proj.transaction?.contractDuration || '',
                            bookingAmount: proj.transaction?.bookingAmount || '',
                            downPayment: proj.transaction?.downPayment || '',
                            monthlyInstallment: proj.transaction?.monthlyInstallment || '',
                            tenure: proj.transaction?.tenure || '',
                            totalPayable: proj.transaction?.totalPayable || '',
                            interestRate: proj.transaction?.interestRate || '',
                            additionalCharges: proj.transaction?.additionalCharges || '',
                            discountsOffers: proj.transaction?.discountsOffers || '',
                            additionalInfo: proj.transaction?.additionalInfo || '',
                        },
                        images: proj.images || [],
                        video: proj.video || '',
                        ...
                        contact: {
                            ...
                            number: proj.contact?.number || '',
                            whatsapp: proj.contact?.whatsapp || '',
                            cnic: proj.contact?.cnic || '',
                            city: proj.contact?.city || '',
                            area: proj.contact?.area || '',
                            companyName: proj.contact?.companyName || '',
                            designation: proj.contact?.designation || '',
                        },
                        declarationAccepted: false,
                    });
                ...
                showToast(response.message || 'Failed to load property', 'error');
            }
        } catch (err) {
            const errorMsg = err.message || 'Network error. Failed to load property.';
            setError(errorMsg);
            showToast(errorMsg, 'error');
        } finally {
    ...
    // Contact info is auto-filled from current user, but can be manually edited
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (id && propertyId) {
                ...
                response = await updateProperty(propertyData);
            } else {
                response = await createProperty(propertyData);
            }
            ...
            if (response.success) {
                ...
                setTimeout(() => navigate('/property/list'), 2000);
            } else {
                const errorMsg = response.message || 'Failed to save property. Please try again.';
                setError(errorMsg);
                showToast(errorMsg, 'error');
            }
        } catch (err) {
                        ...
                        {activeTab === 'contact' && (
                            <div className="space-y-4 xs:space-y-6">
                                ...
                                <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-4 xs:p-6 border-2 border-indigo-200">
                                    ...
                                    <p className="text-[10px] xs:text-xs text-indigo-700 font-medium">
                                        Contact information has been auto-filled from your account. You can edit it below if needed.
                                    </p>
                                </div>
