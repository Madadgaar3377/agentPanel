  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to fetch property applications');
        toast.error(response.message || 'Failed to fetch applications');
        setApplications([]);
      }
    } catch (err) {
      setError(err.message || 'Failed to connect to server');
      toast.error('Failed to fetch property applications');
      setApplications([]);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
          ...
          <p className="text-sm font-medium text-gray-600">Loading property applications...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-red-600 via-red-500 to-rose-600 rounded-3xl shadow-2xl p-8">
          ...
          <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-black text-white tracking-tight">Property Applications</h1>
                <p className="text-red-100 text-sm font-medium mt-0.5">User applications for properties assigned to you</p>
              </div>
            </div>
            <button
              ...
              >
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-lg font-black text-gray-900">Property</p>
        ...
        {filteredApplications.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900">No Property Applications</h3>
            <p className="text-sm text-gray-500 mt-1">
              {searchTerm || statusFilter
                ? 'Try adjusting your search or filters'
                : 'No property applications assigned to you yet'}
            </p>
          </div>
        )}
      </div>
